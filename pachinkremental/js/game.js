...
function InitState() {
	let state = {
		...
		save_file: {
			...
			active_machine: kBumperMachineID,
			stats: {
				score_last5s: 0,
				score_last15s: 0,
				score_last60s: 0,
...
function UpdateScoreDisplay(state, force_update) {
	...
	if (update) {
		let html =
			'<span class="messageBoxLarge">Total bonus time: ' +
...
function YayoInit(){
	 state.machines[state.active_machine_index].upgrades["unlock_combos"].Buy();
	 document.getElementById('upgrades_contents').style['height'] = '0px';
}
...
function Load() {
	...
	if (loaded_save && ActiveMachine(state).GetSaveData().points) {
		...
		document.getElementById("message_box").innerHTML =
			"<h1>Yayo Marathon Bonus Time Pachinko v1</h1>" +
			"<h2>Based on Pachinkremental by Poochy.EXE</h2>" +
			"<h2>All targets and bumpers increase the ball's combo!</h2>" +
			"<h2>Combo gets applied instantly to each target and bumper hit.</h2>" +
			"<h2>Click anywhere in the green box to drop a ball.</h2>" +
			"<h1>GOOD LUCK</h1>";
	}
